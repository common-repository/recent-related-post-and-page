...
?>
<?php 
wp_register_style( 'rr_page_short1_style', false );
wp_enqueue_style( 'rr_page_short1_style' );
$css = " ";
ob_start(); ?>
  @import url(<?php echo esc_url(WL_RP_PLUGIN_URL.'css/all.min.css');?>);
...
<?php
$css .= ob_get_clean();
wp_add_inline_style( 'rr_page_short1_style', $css ); ?>

<?php 
wp_register_style( 'rr_page_short1_style1', false );
wp_enqueue_style( 'rr_page_short1_style1' );
$css = " ";
ob_start(); ?>
    <?php echo esc_attr($nwt_custom_css); ?>
<?php
$css .= ob_get_clean();
wp_add_inline_style( 'rr_page_short1_style1', $css ); ?>

<?php
wp_register_script( 'rr_page_short1_script', false );
wp_enqueue_script( 'rr_page_short1_script', 'src="http://ajax.googleapis.com/ajax/libs/webfont/1/webfont.js"' );
$js = " ";
ob_start(); ?>
...
<?php
$js .= ob_get_clean();
wp_add_inline_script( 'rr_page_short1_script', $js ); ?>
  <?php if ( is_singular( 'page' ) ) { ?> <h1 id="page_post_info_<?php echo esc_attr( $p_o_s_t ); ?>"
    class="weblizar_page_post_info"
    align="center"><?php esc_html_e( $p_p_lbl_text_page, 'WL_R_R_P' ); ?></h1> <?php } ?>

    <?php if ( is_singular( 'post' ) ) { ?> <h1 id="page_post_info_<?php echo esc_attr( $p_o_s_t ); ?>"
      class="weblizar_page_post_info"
      align="center"><?php esc_html_e( $p_p_lbl_text_post, 'WL_R_R_P' ); ?></h1> <?php } ?>
      <div id="slider_div_<?php echo esc_attr( $p_o_s_t ); ?>" class="vticker_<?php echo esc_attr( $p_o_s_t ) . $a; ?> , WL_R_R_P ">
       <?php
	// The Query
       $args      = array(
      ...
      <div id="outer_div_<?php echo esc_attr( $p_o_s_t ); ?>" style="width:100%">
         ...
         while ( $the_query->have_posts() ) { ?>
          <div id="mytemplate_<?php echo esc_attr( $p_o_s_t ); ?>">
           ...
           if ( $count_content > 600 ) {
            ...
            <?php $my_image = wp_get_attachment_url( get_post_thumbnail_id( $post_info ) ); ?>
            <?php if ( ! $my_image == '' ) { ?>
              <div id="r_r_post_image_<?php echo esc_attr( $p_o_s_t ) ?>" class="activity_rounded">
                <?php // echo esc_url("<img src='$my_image'>"); ?>
             ...
             <div id="r_r_post_title_<?php echo esc_attr( $p_o_s_t ) ?>" <?php if ( $my_image == "" ) {
              echo "style='margin-right: 10px !important;margin-left: 10px !important;'";
               ...
               the_permalink();
                 ...
                 esc_html_e('#');
               } ?>" class="web_ticker_title"> <?php if ( $title == "" ) {
                esc_html_e($title = "(no title)", 'WL_R_R_P');
              } else {
                esc_html_e( $title, 'WL_R_R_P' );
              } ?></a></div>
              <div id="r_r_post_content_<?php echo esc_attr( $p_o_s_t ) ?>"
               class="web_ticker_content" <?php if ( $my_image == "" ) {
                echo "style='margin-right: 10px !important;margin-left: 10px !important;'";
              } ?>><?php esc_html_e( $content , 'WL_R_R_P') ?></div>
       ...
       <a id="up_<?php echo esc_attr($p_o_s_t); ?>" class="carousel-control-up  upper_<?php esc_html_e($p_o_s_t . $a); ?>" href="#"
         data-slide="prev"><i class=" fas fa-chevron-up"></i>
       </a>
       <a id="down_<?php echo esc_attr($p_o_s_t); ?>" class="carousel-control-down downer_<?php esc_html_e($p_o_s_t . $a); ?>" href="#"
           data-slide="next"><i class="fas fa-chevron-down"></i>
       </a>
      </div>